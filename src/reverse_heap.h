...
template<class wtype_t>
class reverse_heap {
    const size_t N;
    std::vector<wtype_t> W;  // weights
...
public:
    ...
    void push_heap(int elem, wtype_t w) {
    ...
    inline wtype_t get_weight(int elem) {
    	return W[elem];
    }

    inline wtype_t top_weight() {
        return W[heap[0]];
    }

    // change the weight of an element already in the heap
    void update_weight(int elem, wtype_t w) {
        int i = rev_heap[elem];
        assert(i >= 0 && i < heap.size());
        W[elem] = w;
        percolate_up_or_down(i);
